...
class NatNetClient:
    def __init__( self, body_id_drone1, body_id_patch):
        # Change this value to the IP address of the NatNet server.
        self.serverIPAddress = "192.168.1.111"  #"132.68.36.158" 
        ...
        self.body_id_patch = body_id_patch
    ...
    def __unpackRigidBody( self, data):
        ...
        if self.rigidBodyListener is not None and id == self.body_id_drone1:
            self.rigidBodyListener[0] = np.array([id, np.array(pos), np.array(rot)], dtype='object')

        if self.rigidBodyListener is not None and id == self.body_id_patch:
            self.rigidBodyListener[1] = np.array([id, np.array(pos), np.array(rot)], dtype='object')

...
anim_dir = os.path.join(viz_dir, 'anim_xy_mean')
...
for i in range(len(x_GT_mean)):
    # Plotting the Graph
    for j in range(i+1):
        plt.plot(y_GT_mean[j], x_GT_mean[j], marker='o',
                 markersize=radius_GT_mean[j], color='b')
    for j, xy in enumerate(zip(y_GT_mean[:i+1], x_GT_mean[:i+1])):
        plt.annotate('%d' % j, xy=xy)
    if i > 0:
        for j in range(i):
            plt.plot(y_pred_mean[j], x_pred_mean[j], linestyle="--",
                     marker='o', markersize=radius_pred_mean[j], color='r')
        for j, xy in enumerate(zip(y_pred_mean[:i], x_pred_mean[:i])):
            plt.annotate('%d' % (j+1), xy=xy)
    if i < len(x_GT)-1:
        for j, xy in enumerate(zip(y_planned_mean[:i+1], x_planned_mean[:i+1])):
            plt.plot(y_planned_mean[j], x_planned_mean[j],

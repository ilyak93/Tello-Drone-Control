...
anim_dir = os.path.join(viz_dir, 'anim_xz_mean')
...
x_start, z_start = -700, 260
target_x, target_z = -100, 170
x_stop = -100
...
for i in range(len(x_GT)):
    # Plotting the Graph
    for j in range(i + 1):
        plt.plot(x_GT_mean[j], z_GT_mean[j], marker='o',
                 markersize=radius_GT_mean[j], color='b')
    for j, xz in enumerate(zip(x_GT_mean[:i + 1], z_GT_mean[:i + 1])):
        plt.annotate('%d' % j, xy=xz)
    if i > 0:
        for j in range(i):
            plt.plot(x_pred_mean[j], z_pred_mean[j], linestyle="--",
                     marker='o', markersize=radius_pred_mean[j], color='r')
        for j, xz in enumerate(zip(x_pred_mean[:i], z_pred_mean[:i])):
            plt.annotate('%d' % (j + 1), xy=xz)
    if i < len(x_GT) - 2:
        for j, xz in enumerate(zip(x_planned_mean[:i + 1], z_planned_mean[:i + 1])):
            plt.plot(x_planned_mean[j], z_planned_mean[j],
                     marker='o', markersize=radius_planned_mean[j], color='g')
        for j, xz in enumerate(zip(x_planned_mean[:i + 1], z_planned_mean[:i + 1])):
            plt.annotate('%d' % j, xy=xz)
